...
int libsmtp_connect (char *libsmtp_server, unsigned int libsmtp_port, unsigned int libsmtp_flags, struct libsmtp_session_struct *libsmtp_session)
{
  ...
  buffer_size = snprintf (libsmtp_session->buffer, 4095, "EHLO %s\n", libsmtp_temp_buffer);
  ...
  if (libsmtp_session->lastResponseCode < 300)
  {
    ...
    if (strstr (libsmtp_session->lastResponse, "8BITMIME"))
      libsmtp_session->serverflags || LIBSMTP_HAS_8BIT;

    if (strstr (libsmtp_session->lastResponse, "PIPELINING"))
      libsmtp_session->serverflags || LIBSMTP_HAS_PIPELINING;

    if (strstr (libsmtp_session->lastResponse, "DSN"))
      libsmtp_session->serverflags || LIBSMTP_HAS_DSN;

    if (strstr (libsmtp_session->lastResponse, "STARTTLS"))
      libsmtp_session->serverflags || LIBSMTP_HAS_TLS;

    if (strstr (libsmtp_session->lastResponse, "AUTH"))
      libsmtp_session->serverflags || LIBSMTP_HAS_AUTH;

    if (strstr (libsmtp_session->lastResponse, "SIZE"))
      libsmtp_session->serverflags || LIBSMTP_HAS_SIZE;

    if (strstr (libsmtp_session->lastResponse, "ETRN"))
      libsmtp_session->serverflags || LIBSMTP_HAS_ETRN;

    if (strstr (libsmtp_session->lastResponse, "ENHANCEDSTATUSCODES"))
  ...
  else
  {
    ...
    buffer_size = snprintf (libsmtp_session->buffer, 4095, "HELO %s\n", libsmtp_temp_buffer);
